...
### 设定讨论面板「最近归档讨论」下钻：独立分区 + 可点进完整对话详情（D13 §6.9，后端 + 前端）

- **需求（用户反馈）**：前端「最近归档讨论」原本只是一个 `<details>` 收起的摘要块（下转，非单独列出），且点了不能进入归档讨论的**详细展开**。期望：归档讨论部分有**总结输出**常驻，并且**可点击走进完整对话**。
- **实现（后端读口）**：`src/workbench/discussion.py` 新增 `archived_discussion_detail(novel_root)`——确定性、无 LLM、全缺安全。索引 `config/design_session.yaml` 只存摘要 + `session_file`；完整对话落盘 `book/setting/sessions/session_<ts>.md`（及同名 `.yaml`）。读口：
  - 优先读结构化 `.yaml`，把 `events` 规整为 `blocks`（`discussion`：初始想法 + 逐轮 author/agent + 归纳方向；`summary`：world/scopes/characters/special；menu/supplement）；
  - 仅存 `.md` 时回退给原文 markdown（`raw_markdown`）；全缺只回索引摘要。
  - `session_file` 为相对 data_root 路径 → 相对 novel_root 解析并 `resolve().is_relative_to` 校验落在 novel_root 内（防越权读）。
- **实现（Web）**：`web/api/routers/novels.py` 增 `GET /api/novels/{slug}/discussion/archive`。
- **实现（前端）**：`#discussionBox` 内把归档讨论**单独成区**（`border-top` 分隔），总结输出（summary + last_updated）**常驻**；「查看归档详情 ▾ / 收起 ▴」按钮点击下钻拉取完整对话并展开（首次展开才拉取，缓存跨轮询保留，切小说按 slug 重置 `_archiveOpen/_archiveHtml`）。
- **测试**：`test_workbench_discussion.py` 增 3 条——`.yaml` 结构化 blocks / 仅 `.md` 回退 / 仅索引安全。`node --check` 抽内联 script 通过。全量回归 **496 通过 + 1 跳过**。
...
- **根因**：`run_novel_with_author` 的简介门仅在**无 `config/design_session.yaml`** 时触发；有既有设定/会话（走「是否保留现有设定」覆盖确认）时该门被跳过，`run_design_phase` 首轮重生成以 `reference=None` 运行，无种子。
- **修法（`src/author_loop/design_phase.py`）**：在**作者确认覆盖重生成**（`confirm != y`）分支内、即将 `agent.run` 前，若 `synopsis` 为空且本会话未填过（`_author_synopsis_seeded` 一次性标志），用 `session.read_line` 引导「请先填一句简介作为重新生成设定的初始种子」，非空则 `write_synopsis` 持久化到 `meta.yaml`（供讨论面板读取）并作 `reference` 种子。仅覆盖重生成（危险操作）时触发；**保留/初次无既有设定**路径不受影响（不新增交互）。
- **测试**：`test_design_phase_seed.py` 增 `regen_without_synopsis_prompts_seed`——mock `_has_existing_setting_output=True`、覆盖确认返 `n`、世界已填（避开意图询问）、`synopsis=None`，断言 `agent.run` 的 `reference` 等于补填简介且 `read_synopsis` 已落盘；既有 `test_design_phase.py` 无既有设定用例输入队列不受影响。全量回归 **490 通过 + 1 跳过**。
...
- **后端 ① 在线书名重命名（`src/workbench/novels.py` + `web/api/routers/novels.py`）**：
  - `rename_novel(project_root, slug, title)`——校验标题非空；`slugify_title(title)`（复用 `novel_identity.slugify_title`，与主流程命名口径一致）生成新 slug，目录名冲突递增后缀 `-2`；目录重命名（`novel_root.rename`）后同步三处元数据：`meta.yaml`（title+slug，status 不变：draft 仍 draft）、`data/novels/index.yaml`（按**旧 slug** 移除旧行、按新 slug 追加 `{slug,title,status,updated_at}`）、仓库级 `config/current_novel.yaml`（指针指向本 slug/root 时更新 slug/title/root，`provisional=False`）。
  - **只动身份元数据，不触碰 `book/` 与 `config/*.yaml`（world/characters/outline 写作产物）**——与 `novel_identity._finalize_draft_novel_identity` 断链，纯服务层可单测。
  - Router：`PATCH /api/novels/{slug}/rename`（body `{title}`），校验后返回新 `{slug,title,root}`。
- **后端 ② LLM 工作参数可写（`src/workbench/system.py` + `web/api/routers/system.py`）**：
  - `_framework_info` 判 `readonly` 由 v1 `True` 升 **`False`**（可写白名单）；`_LLM_WRITABLE` = `{model:int? no str, base_url:str, timeout:int/float 1..300, max_retries:int 0..10, api_key_env:str}`（于 `patch_system` 校验类型/范围）。
  - **关键写口差异**：运行时组（author_workbench/internet_search）写 per-novel `config/runtime.yaml` **顶层 `runtime:`** 下（沿用 G3/G4c）；LLM 组写 **顶层 `framework:`**（`_write_framework_override`）——因为 `_framework_info`/`load_runtime_config` 读的是 **top-level `framework`**，而 e2e O per-novel `runtime.yaml` 顶层本就是 `setting_research/agents/runtime`（无 `runtime:` 外层 wrapper）。深合并保留既有 `framework.llm` 与其它 llm_options。
  - ⚠ 诚实标注：`api_key_env` 写的是**环境变量名**（非密钥明文），改后须该环境变量已在 `.env`/shell 提供，否则下一会话真实 LLM 断链——不静默兜底。生效边界不变：下一会话/重启生效。
  - Router `SystemPatchBody` 增 `framework`；`patch_system` 白名单过滤后转发。
- **前端（`web/static/index.html` 纯前端）**：总览/进度页当前书名词条内嵌**可编辑输入 + 保存**（PATCH rename，成功后重载作品索引——slug 可能因命名变化）；「系统」分区 LLM 卡由只读改**白名单子键可编辑**（model/base_url/timeout/max_retries/api_key_env），保存 PATCH `/api/system` 后 GET 刷新。
- **doc-first**：设计 §6.4（LLM v2 可写行 + 写口说明）、§6.8（在线书名编辑 + LLM 可写）、§7 GG5 行、修订记录；next-iteration 当前焦点 G4 末；WORKLOG 本节 + 速览。
- **测试**：`test_workbench_novels.py` 增 `rename_novel` 单测（改名落 meta/index/current_novel、slug 变化目录改名、非空校验）；`test_system.py` 更新 `test_framework_readonly_is_ignored` → `test_framework_writable_whitelist`（model/timeout/max_retries 落盘 + 保留既有 llm + 类型/越界拒绝）；TestClient 路径（`test_web_*`）覆盖 `PATCH /api/novels/{slug}/rename` 与 `/api/system` framework；全量回归保持绿（无 LLM、无真实密钥写盘）。

### 简介种子 + 设定讨论/设定情况面板（§6.9；角色续写工作台）

- **需求**：用户要求「小说命名后，理应引导作者去填写一个**简介**；设定讨论时，应该由该简介先**初始化**一个设定；另外界面上应显示本阶段的**设定情况**，而不是让作者看终端输出（终端输出是出错时可以详细查看的）」。
- **范围（已确认）**：简介作**设定讨论种子**、不重排阶段（设定在前、命名在后；新小说进作者在环先引导填简介）；控制台面板展示**确定性设定快照 + 阶段 + 简介 + world + 设定方向 + 最近已归档讨论摘要**，终端 `<details>` 降级为出错详查。
- **后端 A 引擎 synopsis 种子（`src/author_loop/novel_identity.py` + `design_phase.py`）**：
  - meta.yaml 三处写 dict（`_ensure_provisional_novel_directory`/`_finalize_draft_novel_identity`/`persist_novel_identity`）均加 `synopsis`（空串默认）；`_finalize_draft_novel_identity` **整表重建 meta** 时从旧 meta 保留既有 `synopsis`。新增 `read_synopsis(novel_root)`/`write_synopsis(novel_root, text)`（读写 meta.yaml `synopsis`，非空截断 500 字）。
  - `run_novel_with_author.main` 在 `ensure_current_novel_for_design_phase` 之后、`run_design_phase` 之前：本小说 `synopsis` 为空**且尚无 `config/design_session.yaml`**（避免续跑/讨论中途打扰）时用 `input_fn` 引导「请填写小说简介（一句话设定，可回车跳过）：」，非空则写回 meta 并作为种子；本轮不重复问。
  - `run_design_phase(..., synopsis=None)` 初始化 `reference = synopsis or None`；`_prompt_author_intent_for_setting_research` 增 `seed` 参——作者给了 idea → `seed+"\n"+idea`，回车 → `seed`，无 seed → 原占位。
- **后端 B 确定性读口（`src/workbench/discussion.py::discussion_snapshot` + `web/api/routers/novels.py`）**：`phase`（`author_interaction_state.yaml` phase_state）+ `synopsis` + `world`（复用 `src.config` world）+ `settings`（`setting_research_output.yaml` 每 direction 计数）+ `discussion_summary`（`design_session.yaml`）；全缺安全。Router：`GET /api/novels/{slug}/discussion` + `PATCH /api/novels/{slug}/synopsis`（body `{text}` 写 meta）。
- **前端（`web/static/index.html` 纯前端）**：控制台 `#secConsole` sessionBox 后、termBox 前插 `#discussionBox`，`loadDiscussion()` 渲染阶段徽标/简介（可编辑+保存）/world/设定方向/最近归档讨论，`runDim("console")` + `renderSession` 轮询后轻刷；termBox 外包 `<details>`（默认收起，出错时展开）。
- **诚实标注**：设定**正在进行的单轮**在引擎线内存（仅归档/menu 边界落盘），读口呈现最后一次**已归档** round + 累积 summary；不做实时单轮 re-thread。
- **doc-first**：设计 §6.9 + §7 行 + 修订记录；next-iteration 当前焦点 G4 末；WORKLOG 本节 + 速览。
- **测试**：`write_synopsis`/`read_synopsis` round-trip + `_finalize` 保留 synopsis + `novel_meta` 透出；`run_design_phase(synopsis=...)` 首轮 reference 含词；`discussion_snapshot` 各段 + 缺文件安全；TestClient GET/PATCH；`node --check`；全量回归保持绿（无 LLM）。
...
### 工作台两栏布局 + 控制台「终端输出 / 小说正文」（前端布局 + 确定性读口；GG-W #6）

- **需求**：工作台原先选中一本小说后把 `进度/关系/人物/节拍/控制台/系统` 全部纵向堆叠同时显示；用户要求**左导航栏列各维度、右侧呈现所选维度数据**，且切到**控制台**时右侧同时呈现**整体终端输出**与**小说正文情况**。
- **落地**：
  - **前端（`web/static/index.html` 纯前端两栏）**：`<main>` 改两栏 CSS grid——左维度导航（总览/进度/关系/人物/章节节拍/**控制台**/系统）+ 右内容区只显示激活维度（`showDim`，其余隐藏），**懒加载**（切维度才拉数据），默认「总览」；header 保留小说下拉。控制台维度右侧三区：① 作者在环 `sessionBox`（G4c 既有）② **终端输出**（`session.state().stream` 等宽滚动，控制台激活才轮询）③ **小说正文**（`/api/novels/{slug}/story` 按 turn 陈列 summary+body + 当前章/拍指针）。
  - **后端 A 终端输出 tail（`web/api/session_runner.py`）**：`WorkbenchSession.start()` 给**根 logger** 挂 `_StreamTailHandler`（定长 deque≈300 行，内存态）缓冲引擎日志尾部；`_run_guard` finally / `abort()` 移除（防泄漏）；`state()` 增 `stream`。只读观测，**不改变在环/互斥语义**（无会话不挂、异常也不失败）。
  - **后端 B 小说正文 read port（`src/workbench/novels.py::story_events` + `web/api/routers/novels.py` GET `/api/novels/{slug}/story`）**：复用 `file_sync.load_scope_events_from_disk`，按 turn 倒序取 `summary+body`（`## 正文` 段）+ present_characters/time/place，配 `outline_pointer` 作当前位置；不触碰正文磁盘。
- **诚实标注**：正文以 scope 事件 `## 正文` 段为准；测试/样例小说多只有摘要（body 空），真小说才有正文章节。实时 LLM 流式/彩色终端不在本切片。
...
### 进度维度「未开始 → 引导进入设定讨论阶段」（前端，2026-09-01）

- **需求**：小说世界尚未开始时，「进度」维度应提示需进入**设定讨论阶段**，并提供**跳转链接**。
- **落地**（`loadSummary`）：未开始判定 `!outline_pointer.had_any_beat && scope_event_count<=0`（无大纲节拍且无事件）→ 在进度面板顶部渲染提示卡「尚未开始 · 需进入设定讨论阶段」，说明需完善世界观/设定，并给 `前往控制台 · 设定讨论` 按钮——跳 `showDim('console')`（控制台即作者在环/设定讨论入口；开始作者在环即发起 `run_design_phase`）。已开始的小说不显示（进度摘要照旧）。
- **验证**：`node --check` 通过；服务按请求读磁盘，刷新浏览器即生效。
- 全量单测不受前端影响（468 passed + 1 skipped）。
...
## 2026-08-31（工作台面板 #4 L1记忆 / 屏外线时间线）

### 前端人物卡「L1 记忆 + 屏外线」两节时间线（GG-W #4，doc-first D13 §6.6）

- **背景**：`memories_l1`（L1 事实已落盘 `[{turn_file,summary,layer}]`）与 `off_screen_threads`（`[{summary,thread?,turn_index?,scope_id?,consumed?}]`）各只有一行计数。推进 #4 —— 两节纯前端时间线。
- **方案（doc-first，D13 §6.6 + §7 GG-W 行）**：人物卡增两节——
  - **L1 记忆**：按 `turn_file` 升序，条目 summary（截 120 字）+ `layer` 徽标；
  - **屏外线**：每条 `已消费/待消费` 徽标 + `#turn_index` + `thread` 标签 + `scope_id` + summary。
  - `runtime_only_layers` 标注保留（诚实标注 L2/L3 未落盘）；**无新增后端读口**（零后端改动，default 不破）。
- **验证**：`node --check` 全部 inline JS 通过；`memSection`/`offSection` 排序与徽标样例逻辑核对正确；全量 **450 通过 + 1 跳过**。
- **下一步**：**#5 跨卡联动**（点人物卡/关系节点联动刷新成长·记忆·视野·屏外·节拍各卡）。

---

## 2026-08-31（工作台面板 #5 跨卡联动）

### 共享焦点 `focusPerson` —— 人物卡 ⇄ 关系图谱联动（GG-W #5，doc-first D13 §6.6）

- **背景**：人物卡（成长/记忆/视野/屏外/雷达/时间线）与关系图谱各卡独立加载，缺「点谁看谁」的焦点联动。推进 #5 收口 GG-W 面板系列。
- **方案（doc-first，D13 §6.6 + §7 GG-W 行）**：共享焦点 `focusChar`——
  - **点人物卡标题 / 关系图节点** → `focusPerson(id)`：人物卡 `.focus`（accent 边框+inset 光环）高亮、其余 `.dim` 降强调度、滚入视野；同时把图表 `egoCenter` 设为该角色并重绘 **ego 一跳心图**（关系图谱联动）。
  - 「**全部角色**」`clearFocus()` 复位全 grid + 全书谱。
  - `markFocus()` 纯 DOM class 切换（不重新请求，default 不破）。
- **边界（诚实）**：**节拍/情志条为章节级场景数据**（outline beat 无按角色在场指针），故不走人物焦点过滤；联动作用域为**人物卡 ⇄ 关系图谱**（皆数据就绪的确定性 pane）。角色级节拍过滤需新增 beat→characters 读口，留后续。
- **验证**：`node --check` 全部 inline JS 通过；全量 **450 通过 + 1 跳过**。
- **下一步**：GG-W 面板系列（#2-#5）齐 ✅→ 转主线（CC-c 上下文压缩挂载 / 任务 E / I6）。

---

## 2026-08-31（CC-c 上下文压缩链路挂载）

### 确定性结构保留压缩挂载设计讨论检索组装（D8 §6.1，doc-first）

- **背景**：CC-b 的 `CompressionContract`（契约生成器）只有单测消费、未进检索链路；`retrieve_for_intent` 组装的检索块 `_enforce_total_budget` 是硬截断，缺任务锚定/结构保留的压缩。推进 CC-c 挂载。
- **定策（AskUserQuestion）**：压缩实现选**确定性结构保留**（与 CC-b 一致、DummyLLM guard 不破/default 不破，不引入检索热路径 LLM 调用）；原文「分块压缩提示模板」的 LLM 抽取向移 **CC-d**（可选 A/B）。
- **新增**：
  - `context_compression.reduce_snippet_structure_preserved(text,max_chars)`：**结构保留削减**——保留首个结构化标题行（`#`/`**`/编号/`【`）作锚点 + 逐行装正文行，末尾打「…（已压缩）」节流标记，绝不塌成单段结论；极小额连标题也截锚点（绝不让整块留在原长）。
  - `retrieve_for_intent.compress_retrieval_snippets(snippets,cap,contract,settings)`：**门限触发**——`Σlen ≥ threshold_ratio×cap`（默认 **0.75**）且 `context_compress.enabled` 才压缩，逐块结构保留削减至 **target_ratio×cap**（默认 **0.5**，滞回；保底份额保证非单段结论），来源标签由 assembler 保留 → 输出**仍分块/分节带【来源】**；未超阈/未启用**原样返回**（无阈行为不变）。`_enforce_total_budget` 仍作硬保险。
  - `retrieve_for_intent.load_compress_settings(runtime_config)`：`runtime.author_interaction.context_compress` `{enabled:false, threshold_ratio:0.75, target_ratio:0.5}` 默认关（用户可调开）。
  - **挂载**：`design_phase._assembled_context_for_discussion`（设定自由讨论检索组装）——`build_compression_contract(phase=DESIGN_DISCUSSION,intent,user_input,query,confidence)` → 超阈则 `compress_retrieval_snippets` → assemble（设计完成/兜底 intent 跳过）。主篇检索路径（`cli.py`/`turn_planning`）挂载留后续。
- **可观测**：触发时 `logger.info` 记 `is_compressed chars→cap target contract_proto`。
- **测试**：`test_context_compression.py` 增 4 条 reducer（保留标题骨架/极小额截锚/装得下原样/剪正文保标题）+ `test_retrieve_for_intent.py` 增 4 条 compressor（超阈启用削减且保来源分块/未启用 identity/未超阈 identity/设置默认与覆盖）。全量 **458 通过 + 1 跳过**。
- **修坑**：`reduce` 早期 `max_chars < 16 原样返回` 会在 target 很小时把压缩变成 no-op → 改为极小额也截锚点，保证真压缩。另发现 `retrieve_for_intent ⇄ author_harness` 存在**既有**（非本次引入）循环依赖，测试以先 import `author_harness` 规避。
- **下一步**：主篇检索路径（turn_planning / cli）挂载 CC-c；或转任务 E / I6。**安全强化（#31）留最后。**
