...
- **上下文压缩（D8，组装后；与 `retrieve_for_intent` 硬截断衔接）**：
  ...
  - **CC-c（✅ 2026-08-31，确定性结构保留压缩）**：`context_compression.reduce_snippet_structure_preserved`（结构保留削减：保留标题骨架+逐行装正文，禁止单段结论）+ `retrieve_for_intent.compress_retrieval_snippets`（门限 0.75×cap 触发、滞回 0.5×cap、默认关 `context_compress.enabled=false`）+ `load_compress_settings`；挂载 `design_phase._assembled_context_for_discussion`（设定讨论检索组装）：`build_compression_contract` → 超阈压缩 → assemble，输出仍分块带【来源】；未超阈/未启用原样（无阈行为不变）。LLM 抽取向移 **CC-d**（D8 §6.1）。
...
- **远程访问安全强化（GG6 后续；自用上岸后统一做，当前只做 Basic Auth 密码登录）**：
  - **当前定策（2026-08-31，用户确认自用）**：工作台主要**自用**，安全整体后置；现已有 `config/web_api.yaml` `auth.enabled` **Basic Auth 密码登录**（默认关）+ 可配 `server.host/port` + 空口令 fail-fast，够单作者局域网/反代后自用。
  - **后期（不立项即可按需做）**：**反代前置（nginx/caddy + 更强认证 / OAuth）**、**HTTPS/TLS**、**口令存储最小化**（明文 → 哈希/环境变量/密钥，避免进 git）、**会话/限流**、可能 **CORS 白名单**。验收：对外服务前至少开 auth + 反代 + HTTPS，杜绝写口裸奔。
- **新小说链路 E2E 验收（任务 E）**：
