...
- **上下文压缩（D8，组装后；与 `retrieve_for_intent` 硬截断衔接）**：
  ...
  - **CC-b**：`CompressionContract` 数据结构 + 规则/软原型生成器（可选轻量 LLM）；单测多意图与兜底路径。
