...
**类型**：SDD（L2）。**登记**：**[SPEC_SDD.md](../framework/SPEC_SDD.md) D10**。**状态**：**G1a 已编码 ✅、G1b 已编码 ✅、G1c 已编码 ✅**（2026-08-31）。
...
1. **屏外数据模型**：每角色可累积 `off_screen` / `parallel_thread` 类型记忆条目（术语定义见 §2），与主书 scope 事件分离、可追溯、可截断。
...
| 术语 | 含义 |
|------|------|
| **屏外线 / off-screen** | **概念层**：角色不在当前场景（非 `present`）时发生的独立活动，主书镜头不照其上但生活照常推进（镜头隐喻：主书以主角为镜头轴，他人处于「镜头外」）。 |
| **屏外数据 / off-screen memory** | **数据层**：指「屏外线」在存储的具体落点 = 该角色 `threads/off_screen.yaml` 中 `thread: off_screen | parallel_thread` 的记忆条目，复用 L1/L2/L3 分层，与主书 `events/` 分离、可追溯、可截断。写入通道 `append_off_screen_refinement` / `get_recent_off_screen`。 |
...
| **G1c 批处理（可选）** | `trigger=batch`：每 `batch_turns` 扫一次各角色 `threads` 累积演进 | ✅（2026-08-31）：`write_off_screen_threads` + `apply_off_screen_batch`（未消费条目批内回放→标 `consumed` 幂等；`max_entries` 上限）+ `scan_off_screen_batch_for_all` + 回环按 `trigger=batch` 每 `batch_turns` 触发；4 条单测（批次触发、幂等空转、上限分身、扫描入口空转），全量 351 通过 |
